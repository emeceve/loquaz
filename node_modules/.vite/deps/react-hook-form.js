...
var FormProvider = (props) => {
  ...
  return import_react.default.createElement(HookFormContext.Provider, { value: data }, props.children);
};
...
function useFormState(props) {
  ...
  _name.current = name;
  const callback = import_react.default.useCallback((value) => _mounted.current && shouldSubscribeByName(_name.current, value.name, exact) && shouldRenderFormState(value, _localProxyFormState.current) && updateFormState(__spreadValues(__spreadValues({}, control._formState), value)), [control, exact]);
  useSubscribe({
    disabled,
    callback,
...
function useController(props) {
  ...
  const _registerProps = import_react.default.useRef(control.register(name, __spreadProps(__spreadValues({}, props.rules), {
    value
  })));
  import_react.default.useEffect(() => {
...
function useFieldArray(props) {
  ...
  return {
    ...
    fields: import_react.default.useMemo(() => fields.map((field, index) => __spreadProps(__spreadValues({}, field), {
...
function createFormControl(props = {}) {
  ...
  const shouldRenderByError = async (name, isValid, error, fieldState) => {
    ...
    if (props.delayError && error) {
      delayErrorCallback = delayErrorCallback || debounce(() => updateErrors(name, error));
    ...
    if ((error ? !deepEqual(previousFieldError, error) : previousFieldError) || !isEmptyObject(fieldState) || shouldUpdateValid) {
      const updatedFormState = __spreadProps(__spreadValues(__spreadValues({}, fieldState), shouldUpdateValid ? { isValid } : {}), {
        errors: _formState.errors,
        name
      });
  ...
  const executeBuildInValidation = async (fields, shouldOnlyCheckValid, context = {
    ...
    for (const name in fields) {
      ...
      if (field) {
        const _a = field, { _f: fieldReference } = _a, fieldValue = __objRest(_a, ["_f"]);
        if (fieldReference) {
  ...
  const _getWatch = (names, defaultValue, isGlobal) => {
    const fieldValues = __spreadValues({}, _stateFlags.mount ? _formValues : isUndefined(defaultValue) ? _defaultValues : isString(names) ? { [names]: defaultValue } : defaultValue);
  ...
  const trigger = async (name, options = {}) => {
    ...
    _subjects.state.next(__spreadProps(__spreadValues(__spreadValues({}, !isString(name) || _proxyFormState.isValid && isValid !== _formState.isValid ? {} : { name }), _options.resolver ? { isValid } : {}), {
      errors: _formState.errors,
      isValidating: false
    }));
